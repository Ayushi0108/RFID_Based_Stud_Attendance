...
void setup() {
  ...
  WiFi.begin("Redmi Note 10 Pro","Redmi@Sarthak"); // recommended in setup() instead of loop
}
...
void loop() {
  ...
  if(WiFi.status() != WL_CONNECTED) // Wait if the wifi connection is 
  {
    ...
    WiFi.begin("Redmi Note 10 Pro","Redmi@Sarthak");
  ...
  Link = "http://192.168.112.55:8000/mark/?rfid="+CardID;
  HTTPClient http; //object of HttpClient
  WiFiClient client;
  // http.begin(Link);
  http.begin(client, Link);
  int httpCode = http.GET();
  if(httpCode > 0) // successfull if positive
  {
